<template>
  <button
    ...
    @click="cetakLaporan(tahun, tanggal_cetak)"
  >
    Cetak Laporan
  </button>
  <div
  ...
  >
    <h1 class="h2 m-3">Laporan Kinerja Instruktur</h1>
  </div>
  <div class="container">
    <div class="row">
      <div class="col-md-12">
        <div class="card border-0 rounded shadow">
          <!-- <div class="card-title">
                <div class="d-flex mt-3 ms-3">
                    <select class="form-select select-style" aria-label="Default select example" v-model="month">
                    ...
                    <button class="btn btn-primary ms-3" type="submit" @click="getKinerjaInstruktur(month)">Cari</button>
                </div>
            </div> -->
          <div
...
export default {
  setup() {
    ...
    onMounted(() => {
      ...
      axios
        .get("https://valent.ppcdeveloper.com/api/laporanKinerjaInstruktur")
        .then((response) => {
    ...
    // function getKinerjaInstruktur($month){
    //   axios
    //     .get("http://127.0.0.1:8000/api/laporanKinerjaInstruktur/" + $month)
    //     .then((response) => {
    //       //assign state posts with response data
    //       laporan.value = response.data.data;
    //       bulan.value = response.data.bulan;
    ...
    function cetakLaporan($periode, $tanggal_cetak) {
      ...
      doc.setFont("helvetica", "bold");
      doc.text("Laporan Kinerja Instruktur", 15, 50);
      doc.setFont("helvetica", "normal");
      ...
      doc.save("Laporan Kinerja Instruktur - " + $tanggal_cetak + ".pdf");
    }
